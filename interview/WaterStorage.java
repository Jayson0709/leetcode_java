...
public class WaterStorage {
    public static void main(String[] args) {
        ...
        cin.close();
        System.out.println(storeWater(obj.array1, obj.array2));
    }

    private static int storeWater(int[] bucket, int[] vat) {
        int maxK = 0;
        if (Arrays.stream(vat).max().isPresent()) {
            maxK = Arrays.stream(vat).max().getAsInt();
        }
        if (maxK == 0) {
            return 0;
        }
